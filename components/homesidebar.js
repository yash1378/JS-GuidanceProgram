...
const DataPage = ({
  ...
  const [selectedStudentPhoneNumbers, setSelectedStudentPhoneNumbers] = useState([]); // Updated state variable
  ...
  const toggleSelect = (phoneNumber) => { // Updated parameter name
    if (selectedStudentPhoneNumbers.includes(phoneNumber)) {
      setSelectedStudentPhoneNumbers(selectedStudentPhoneNumbers.filter((phone) => phone !== phoneNumber));
    } else {
      setSelectedStudentPhoneNumbers([...selectedStudentPhoneNumbers, phoneNumber]);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log(selectedMentor, selectedStudentPhoneNumbers)
    try {
      ...
      const response = await fetch(
        "https://gp-backend-u5ty.onrender.com/api/finalMentor",
        {
          ...
          body: JSON.stringify({
            ...
            studentPhoneNumbers: selectedStudentPhoneNumbers, // Updated field name
          }),
        }
      );
  ...
  useEffect(() => {
    // Update the selectedStudentPhoneNumbers whenever selectedRows change
    const phoneNumbers = data
      .filter((item) => selectedRows.includes(item.phone)) // Updated field name
      .map((item) => item.phone);
    setSelectedStudentPhoneNumbers(phoneNumbers);
  }, [selectedRows]);
                  ...
                  .map((item, index) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <input
                          type="checkbox"
                          checked={selectedRows.includes(item.phone)}
                          onChange={() => toggleSelect(item.phone)}
                        />
                      </td>
                      ...
                      <td className="text-white px-6 py-4">  {new Date(item.date).toLocaleDateString()}</td>
                      <td className="text-white px-6 py-4">{item.subject}</td>
                      <td className="text-white px-6 py-4">{item._class}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
